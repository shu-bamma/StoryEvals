...
class VideoShotProcessor:
        ...
        try:
            ...
            if os.path.exists(temp_video_path):
                ...
                if os.path.getsize(temp_video_path) > 0:
                    logger.info(
                        f"Video already exists, skipping download: {temp_video_path}"
                    )
                    return temp_video_path
                else:
                    logger.warning(
                        f"Existing video file is empty, re-downloading: {temp_video_path}"
                    )
    ...
    def detect_shot_boundaries(self, video_path: str) -> list[dict[str, Any]]:
        ...
        scene_manager.add_detector(
            ContentDetector(threshold=self.config["shot_boundary_threshold"])
        )
        scene_manager.add_detector(
        ...
        logger.info(
            f"Detected {len(shots)} shots with {sum(len(s['keyframes_ms']) for s in shots)} total keyframes"
        )
        return shots
    ...
    def process_shots_with_yolo(
        self, video_path: str, shots: list[dict[str, Any]], project_id: str
    ) -> list[Shot]:
        ...
        for shot in shots:
            ...
            for keyframe_ms in shot["keyframes_ms"]:
                ...
                for result in results:
                    if result.boxes is not None:
                        ...
                        for i, (box, conf, cls_id) in enumerate(
                            zip(boxes, confidences, class_ids, strict=False)
                        ):
                            class_name = self.model.names[cls_id]
                            if (
                                class_name in self.target_classes
                                and conf >= self.config["detection_confidence"]
                            ):
                        ...
                        if self.config.get("nms_enabled", True):
                            ...
                            final_confidences = [
                                valid_confidences[i] for i in nms_indices
                            ]
                        ...
                        for box, conf, _cls_id in zip(
                            ...
                            expanded_box = self.expand_bbox(
                                box, frame.shape, self.config["bbox_expansion"]
                            )
                            ...
                            if crop_image.size == 0:
                                logger.warning(
                                    f"Empty crop extracted at {keyframe_ms}ms"
                                )
                                continue

                            # Generate crop ID
                            crop_id = (
                                f"c_{shot['shot_id']}_t{keyframe_ms:04d}_{crop_index}"
                            )
                            ...
                            bbox_norm = [
                                ...
                                float(
                                    (expanded_box[3] - expanded_box[1]) / h
                                ),  # height
                            ]
                ...
                logger.debug(
                    f"Found {len(crops_for_keyframe)} crops at keyframe {keyframe_ms}ms"
                )
            ...
            logger.info(
                f"Completed shot {shot['shot_id']} with {total_crops} total crops"
            )
        ...
        total_crops = sum(
            sum(len(crops) for crops in shot.crops.values()) for shot in processed_shots
        )
        logger.info(
            f"YOLO processing complete: {total_shots} shots, {total_crops} total crops"
        )
    ...
    def create_debug_video(
        ...
        debug_output_path = os.path.join(
            self.config["temp_video_dir"], f"{project_id}_debug.mp4"
        )
        ...
        try:
            ...
            while True:
                ...
                for shot in shot_boundaries:
                    if (
                        abs(current_time_ms - shot["start_ms"]) < 500
                    ):  # Within 500ms of boundary
                        cv2.line(frame, (0, 0), (width, height), (0, 0, 255), 3)
                        cv2.putText(
                            frame,
                            f"SHOT START: {shot['shot_id']}",
                            (10, height - 30),
                            cv2.FONT_HERSHEY_SIMPLEX,
                            1,
                            (0, 0, 255),
                ...
                for keyframe_timestamp, crops_list in crops_by_timestamp.items():
                    ...
                    if abs(current_time_ms - keyframe_timestamp) <= 210:
                        ...
                        cv2.putText(
                            frame,
                            f"Keyframe: {keyframe_timestamp}ms",
                            (10, 110),
                            cv2.FONT_HERSHEY_SIMPLEX,
                            0.6,
                            (255, 255, 0),
    ...
    def process_video(
        ...
        video_url: str,
        project_id: str,
        character_vault: list[dict[str, Any]],
        ...
        try:
            ...
            processed_shots = self.process_shots_with_yolo(
                video_path, shots, project_id
            )

            # Step 4: Create debug video if requested
            debug_video_path = None
            if create_debug_video:
                debug_video_path = self.create_debug_video(
                    video_path, shots, processed_shots, project_id
                )
            ...
            logger.info(
                f"Video processing complete for {project_id} in {processing_time:.1f}s"
            )
    ...
    def save_results(
        self, result: dict[str, Any], output_path: str | None = None
    ) -> str:
                ...
                for crop in crops_list:
                    crop_dict = {
                        ...
                        "quality": {
                            ...
                            "pose": crop.quality.pose
                            if crop.quality
                            else None,  # Fix missing pose
                        }
                        if crop.quality
                        else None,
                    }
