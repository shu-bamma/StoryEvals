...
def parse_enhanced_prompt(enhanced_prompt_str: str) -> dict[str, Any]:
    ...
    """
    try:
        result = json.loads(enhanced_prompt_str)
...
def find_best_character_match(
    subject_name: str, available_characters: list[str]
) -> str:
    """
...
def extract_subjects_from_metadata(metadata: list[dict[str, Any]]) -> list[str]:
    ...
    try:
        ...
        enhanced_prompt_data = parse_enhanced_prompt(
            enhanced_prompt_item.get("value", "{}")
        )
...
def save_results_to_json(
    all_results: dict[str, list], output_filename: str = "story_evals_results.json"
) -> None:
    """
...
def save_critique_results_to_json(
    critique_results: dict[str, list],
    output_filename: str = "critique_evaluation_results.json",
) -> None:
    """
    ...
    json_output: dict[str, Any] = {
        "summary": {
            ...
            "total_videos_evaluated": sum(
                len(results) for results in critique_results.values()
            ),
