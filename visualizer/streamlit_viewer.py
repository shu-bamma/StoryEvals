...
from process_results import PipelineResultsProcessor, ProjectData, KeyframeData, CropData
...
class StreamlitViewer:
    ...
    def _auto_extract_keyframes(self):
        ...
        if st.session_state.extraction_in_progress or st.session_state.keyframes_extracted:
            return
    ...
    def _render_timeline_histogram(self, project_data: ProjectData):
        ...
        char_color_map = dict(zip(all_characters, character_colors))
        ...
        fig = make_subplots(
            rows=2, cols=1,
            subplot_titles=("YOLO Detection Count per Keyframe", "Character Identification Timeline"),
        ...
        bar_colors = ['gold' if ts == current_timestamp else 'lightblue' for ts in timestamps]
        ...
        for char_id in all_characters:
            ...
            if char_points:
                ...
                regular_points = [p for p in char_points if p['timestamp'] != current_timestamp]
                current_points = [p for p in char_points if p['timestamp'] == current_timestamp]
                ...
                if regular_points:
                    fig.add_trace(
                        go.Scatter(
                            ...
                            customdata=[[p['yolo_conf'], p['llm_conf']] for p in regular_points],
                            showlegend=True
                        ),
                        row=2, col=1
                    )
                    ...
                    fig.add_trace(
                        go.Scatter(
                            ...
                            marker=dict(
                                color=char_color_map[char_id],
                                size=15,  # Larger size
                                line=dict(width=3, color='gold'),  # Gold border
                            ...
                            customdata=[[p['yolo_conf'], p['llm_conf']] for p in current_points],
        ...
        if unknown_points:
            regular_unknown = [p for p in unknown_points if p['timestamp'] != current_timestamp]
            current_unknown = [p for p in unknown_points if p['timestamp'] == current_timestamp]
        ...
        with col2:
            identified_chars = len([p for p in character_points if p['character'] != "Unknown"])
        ...
        with col4:
            keyframes_with_chars = len(set(p['timestamp'] for p in character_points if p['character'] != "Unknown"))
